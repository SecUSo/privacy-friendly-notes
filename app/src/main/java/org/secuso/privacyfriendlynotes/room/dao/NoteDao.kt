...
interface NoteDao {
    ...
    @Query("SELECT * FROM notes WHERE ((LOWER(name) LIKE '%'|| LOWER(:thisFilterText) || '%') OR (LOWER(content) LIKE '%'|| LOWER(:thisFilterText) || '%' AND (category=1 OR category = 3))) AND in_trash='0' ORDER BY name ASC")
